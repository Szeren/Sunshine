...
public class TestDb extends AndroidTestCase {
    ...
    */
    ...
    public void testWeatherTable() {
        ...
        assertFalse(weatherRowId == -1L);

        // Query the database and receive a Cursor back
        Cursor cursor = db.query(WeatherContract.WeatherEntry.TABLE_NAME,
                null, // all columns
                null, // no row filter
                null, // no args for row filter
                null, // no groupings for rows
                null, // no args for row groupings
                null); // natural order
        ...
        // query if you like)
        TestUtilities.validateCurrentRecord("Error: weather record seems invalid.",
                cursor, contentValues);

        // Assert that the weather table has only the one record we inserted.
        assertFalse("Error: additional records found in weather table.", cursor.moveToNext());
...
//                WeatherContract.LocationEntry.TABLE_NAME + ";", null);
        ...
        Cursor cursor = db.query(WeatherContract.LocationEntry.TABLE_NAME,
                null, // all columns
                null, // all rows (no filter for where clause)
                null, // no args for above where clause
                null, // no row groupings
                null, // no filter for row groupings
                null); // no ordering (natural order)

        // Move the cursor to a valid database row
        assertTrue("Error: No records returned for query to location table.", cursor.moveToFirst());

        // Validate data in resulting Cursor with the original ContentValues
        // (you can use the validateCurrentRecord function in TestUtilities to validate the
        // query if you like)
        TestUtilities.validateCurrentRecord("Error: North pole query seems invalid.",
                cursor, contentValues);

        // Make sure there exists only the record we inserted in the table.
        assertFalse("Error: Additional records found in table.", cursor.moveToNext());
